...
export function SelectStage0() {
  const filteredQuestions = QUESTIONS.filter(question => question.difficulty === "easy");
  return (
...
export function SelectStage1() {
  const filteredQuestions = QUESTIONS.filter(question => question.difficulty === "medium");
  return (
    <div className="flex gap-4 justify-center items-center h-screen">
     <div className="w-32 mr-4 flex flex-col">
        <Link className="flex justify-center items-center border rounded-md p-2 border-gray-500 mb-2" to={url.menu}>
          メニュー
        </Link>
        <Link className="flex justify-center items-center border rounded-md p-2 border-gray-500" to={url.quizEntrance}>
          難易度選択
        </Link>
      </div>
      <div className="font-bold text-center mt-20">
...
export function SelectStage2() {
  const filteredQuestions = QUESTIONS.filter(question => question.difficulty === "hard");
  return (
    <div className="flex gap-4 justify-center items-center h-screen">
      <div className="w-32 mr-4 flex flex-col">
        <Link className="flex justify-center items-center border rounded-md p-2 border-gray-500 mb-2" to={url.menu}>
